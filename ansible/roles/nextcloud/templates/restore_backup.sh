...
if (( $# != 1 )); then
    ...
    echo "        restore_backup.sh nextcloud-backup_20201231"
    exit 1
fi
...
if [[ ! -f "$db_file" && ! -f "$files" ]]; then
    ...
    swift download {{ swift_bucket_name }} $db_file
    swift download {{ swift_bucket_name }} $files
fi
...
rsync -Aax ./nextcloud-files /var/disk1/nextcloud_var_www_html/

/usr/local/bin/docker-compose -f /srv/nextcloud/docker-compose.yml exec --user postgres db dropdb -U {{ POSTGRES_USER }} {{ POSTGRES_DB }}
/usr/local/bin/docker-compose -f /srv/nextcloud/docker-compose.yml exec --user postgres db createdb -U {{ POSTGRES_USER }} -O {{ POSTGRES_USER }} {{ POSTGRES_DB }}
gunzip -c "$db_file" | /usr/local/bin/docker-compose -f /srv/nextcloud/docker-compose.yml exec --user postgres db psql -U {{ POSTGRES_USER }} {{ POSTGRES_DB }}
